...
float MLSRec::estimateSigma(const QImage &img,int x,int y) {
    // estimate sigma at x,y depending on the positions of nearest valid neighbors
      ...
      /* nombre de voisins parcourus mini < voisinage/2 */
      /* le nombre loop max est à modifier selon l'estimation et le pourcentage de bruit */
      while(loop <= 15 && nb_neigh < voisinage/2 ){
          nb_neigh=0;
